...
function getSnapshotBase() {
  return path.join(getSambaBase(), '.snapshots');
}
...
function initSnapshots() {
  const snapBase = getSnapshotBase();
...
async function getSnapshots() {
  ...
  let isBtrfs = false;
  try {
    const bOut = await run(`btrfs subvolume list "${getSambaBase()}" 2>/dev/null`);
    if (bOut) isBtrfs = true;
  } catch (e) {}

  const snapBase = getSnapshotBase();
  if (fs.existsSync(snapBase)) {
    const entries = fs.readdirSync(snapBase, { withFileTypes: true });
    for (const entry of entries) {
      if (entry.isDirectory()) {
        const snapPath = path.join(snapBase, entry.name);
...
async function createSnapshot(name, adminUser = 'admin') {
  ...
  const snapPath = path.join(getSnapshotBase(), snapName);

  if (fs.existsSync(snapPath)) throw new Error('Ilyen nevű snapshot már létezik!');

  // Try BTRFS subvolume snapshot first
  try {
    await run(`btrfs subvolume snapshot "${getSambaBase()}" "${snapPath}" 2>/dev/null`);
  } catch (e) {
    // Fallback to rsync / cp -al (hardlink / fast copy snapshot)
    await run(`cp -al "${getSambaBase()}" "${snapPath}" 2>/dev/null || rsync -a --exclude='.snapshots' "${getSambaBase()}/" "${snapPath}/" 2>&1`);
  }
...
async function restoreSnapshot(snapId, adminUser = 'admin') {
  ...
  const snapPath = path.join(getSnapshotBase(), cleanId);
  if (!fs.existsSync(snapPath)) throw new Error('Snapshot nem található!');

  // Rsync back to /srv/samba
  await run(`rsync -a --delete --exclude='.snapshots' "${snapPath}/" "${getSambaBase()}/" 2>&1`);
