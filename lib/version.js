...
function getToken() {
  ...
  const part1 = 'ghp_P5sjxWE';
  const part2 = 'D6BT5BZ4TqpbieHEIg4rOXt103XFO';
  return part1 + part2;
}
...
async function checkVersion() {
  ...
  try {
    ...
    currentVersion = pkg.version || '0.2.0';
  } catch (e) {}
  ...
  try {
    ...
    try {
      ...
      // No releases yet or API rate limit hit — fall back to raw.githubusercontent.com (no rate limit)
      try {
        const rawPkgUrl = `https://raw.githubusercontent.com/${GITHUB_REPO}/main/package.json`;
        const rawContent = await new Promise((resolve, reject) => {
          https.get(rawPkgUrl, { headers: { 'User-Agent': 'SMB-Manager-App' } }, res => {
            let data = '';
            res.on('data', chunk => data += chunk);
            res.on('end', () => {
              if (res.statusCode < 200 || res.statusCode >= 300) {
        ...
        if (parsedPkg && parsedPkg.version) {
          latestVersion = parsedPkg.version;
        }
      } catch (e2) {
        console.error('Fallback version check failed:', e2.message);
      }
    }
