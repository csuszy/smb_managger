...
async function quickSetupTimeMachineShare({
  ...
  // Set proper permissions and ownership for backup directories
  await run(`chmod 2775 "${targetPath}" 2>/dev/null || chmod 0777 "${targetPath}" 2>/dev/null`).catch(() => {});
  await run(`chown -R csuszy:users "${targetPath}" 2>/dev/null || chown -R 1000:100 "${targetPath}" 2>/dev/null`).catch(() => {});
  await run(`setfacl -b "${targetPath}" 2>/dev/null`).catch(() => {});
