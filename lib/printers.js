  ...
  try {
    ...
    if (targetId) {
      try {
        ...
        logEvent('files', `Fájl kinyomtatva (CUPS): ${path.basename(filePath)} -> ${targetId}`, 'admin');
        notifyEvent('files', '🜁 Nyomtatás Sikeres', `Fájl kinyomtatva: ${path.basename(filePath)} (${targetId})`, 0x06b6d4).catch(() => {});
        ...
        if (manual && manual.ip) {
          ...
          notifyEvent('files', '🜁 Nyomtatás Sikeres', `Fájl kinyomtatva: ${path.basename(filePath)} (${manual.name || manual.ip})`, 0x06b6d4).catch(() => {});
    ...
    try {
      ...
      const result = await run(cmd);
      logEvent('files', `Fájl kinyomtatva: ${path.basename(filePath)} (Alapértelmezett)`, 'admin');
      notifyEvent('files', '🜁 Nyomtatás Sikeres', `Fájl kinyomtatva: ${path.basename(filePath)} (Alapértelmezett)`, 0x06b6d4).catch(() => {});
      return { success: true, message: result };
    } catch (e) {
      throw new Error(`Nem sikerült a nyomtatás. Kérlek válassz ki egy nyomtatót a beállításokban! (${e.message})`);
    }
  } finally {
    // Cleanup tmp file
    try {
