...
// Parse attachments from raw MIME Buffer or string
function extractMimeAttachments(mimeInput) {
  ...
  // Search boundary in header
  const headerPreview = mimeBuffer.slice(0, Math.min(mimeBuffer.length, 8192)).toString('binary');
  const boundaryMatch = headerPreview.match(/boundary="?([^"\r\n;]+)"?/i);
  ...
  if (boundaryMatch) {
    ...
    let pos = 0;
    while (pos < mimeBuffer.length) {
      const partStart = mimeBuffer.indexOf(boundaryBuf, pos);
      if (partStart === -1) break;

      const nextPartStart = mimeBuffer.indexOf(boundaryBuf, partStart + boundaryBuf.length);
      const partEnd = nextPartStart !== -1 ? nextPartStart : mimeBuffer.length;

      const partBuf = mimeBuffer.slice(partStart + boundaryBuf.length, partEnd);
      pos = partEnd;

      const headerEndIdx = partBuf.indexOf('\r\n\r\n');
      if (headerEndIdx === -1) continue;

      const headerStr = partBuf.slice(0, headerEndIdx).toString('binary');
      if (/Content-Disposition:\s*attachment/i.test(headerStr) || /filename=/i.test(headerStr) || /name=/i.test(headerStr)) {
        const filenameMatch = headerStr.match(/filename="?([^"\r\n;]+)"?/i) || headerStr.match(/name="?([^"\r\n;]+)"?/i);
        if (filenameMatch) {
          let filename = decodeMimeHeader(filenameMatch[1].trim().replace(/^["']|["']$/g, ''));
          ...
          if (encoding === 'base64') {
            ...
            for (let i = 0; i < cleanQp.length; i++) {
              if (cleanQp[i] === '=' && i + 2 < cleanQp.length) {
...
// Zero-dependency IMAP client for Email-To-Print with Raw Buffer preservation
function checkImapEmailAccount(emailCfg) {
  return new Promise((resolve, reject) => {
    ...
    socket.on('data', (chunk) => {
      ...
      const textPreview = rawBuffer.toString('binary');
      ...
      for (let i = 0; i < lines.length - 1; i++) {
        ...
        if (currentStep === 0 && line.includes('* OK')) {
          ...
          if (line.startsWith(currentTag)) {
            // Process MIME attachments directly from rawBuffer
            const subjectMatch = textPreview.match(/^Subject:\s*([^\r\n]+)/mi);
            const fromMatch = textPreview.match(/^From:\s*([^\r\n]+)/mi);
            const dateMatch = textPreview.match(/^Date:\s*([^\r\n]+)/mi);
            ...
            const attachments = extractMimeAttachments(rawBuffer);
