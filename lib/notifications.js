  ...
  return new Promise((resolve, reject) => {
    ...
    function step() {
      ...
      if (currentStep === 1) {
        ...
        const mailContent = [
          `From: ${fromEmail || user || 'SambaHub'} <${fromEmail || user || 'noreply@sambahub.local'}>`,
          `To: <${toEmail}>`,
          `Subject: ${subject}`,
...
async function notifyEvent(category, title, message, color = 0x8b5cf6) {
  ...
  if (cfg.smtp && cfg.smtp.enabled && cfg.smtp.host && cfg.smtp.toEmail) {
    promises.push(
      sendSmtpEmail(cfg.smtp, `[SambaHub] ${title}`, message).catch(e => {
