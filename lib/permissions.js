...
async function getFolderPermissions(folderPath) {
  ...
  // 3. Build user permissions list - merge POSIX owner + other + ACL entries
  const userList = users.map(u => {
    ...
    // Apply "other" bits as baseline minimum for everyone
    level = higherLevel(level, posixOtherLevel);
  ...
  // 4. Build group permissions list - merge POSIX group + other + ACL entries
