...
async function saveSambaGlobalConfig(settings, adminUser = 'admin') {
  ...
  globalConf += `   ea support = yes\n`;
  if (settings.vfsRecycleGlobal) {
    globalConf += `   vfs objects = recycle catia fruit streams_xattr\n`;
    globalConf += `   recycle:repository = .recycle\n`;
    globalConf += `   recycle:keeptree = yes\n`;
  } else {
    globalConf += `   vfs objects = catia fruit streams_xattr\n`;
  }
  globalConf += `   fruit:metadata = stream\n`;
  globalConf += `   fruit:model = Macmini\n`;
  globalConf += `   fruit:posix_rename = yes\n`;
  globalConf += `   fruit:veto_appledouble = no\n`;
  globalConf += `   fruit:wipe_intentionally_left_blank_rfork = yes\n`;
  globalConf += `   fruit:delete_empty_adfiles = yes\n`;
  globalConf += `\n`;
